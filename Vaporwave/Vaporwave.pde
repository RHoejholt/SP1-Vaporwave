...
void drawSky() {
  ...
  drawGradient(0, height, 40, 24, 80, 240, 85, 100);
  ...
  drawGradientLines(0, height, 40, 24, 80, 240, 85, 100 );
}
void drawGradient (float startY, float endY, int startR, int startG, int startB, int endR, int endG, int endB) {
  for (int y = int(startY); y < 20*int(endY); y++) {
    float inter = map(y, startY, endY*10, 0, 1);
    color startColor = color(startR, startG, startB);
    color endColor = color(endR, endG, endB);
    color c = lerpColor(startColor, endColor, inter);
    stroke(c);
    line(-82110, 11111+y, 5*width, y+500, -1000, -1000);
  }
}
void drawGradientLines (float startY, float endY, int startR, int startG, int startB, int endR, int endG, int endB) {
